...
GRDC_catalogue %>%
  filter(
    between(x = long, left = -10, right = 5),
...
MOPEX_catalogue %>%
  filter(
    dplyr::between(x = Longitude, left = -95, right = -92),
...
Please note that these data are updated every 15 minutes and the code will always generate different plots. 
```{r SEPA3, eval=FALSE, message = FALSE, fig.width = 7}
# Multiple time series extraction
y <- tsSEPA(id = c("234253", "234174", "234305"))

plot(y[[1]],
  ylim = c(0, max(y[[1]], y[[2]], y[[3]])),
